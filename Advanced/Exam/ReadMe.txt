Data Structures Advanced with C# - Regular Exam
...
If there is no such task - throw ArgumentException()
...
If there is no such executed task - throw ArgumentException()
●	IEnumerable<Task> GetDomainTasks(string domain) – returns all unexecuted tasks, which are in the given domain.
If there are no tasks in the given domain - throw ArgumentException()
...
●	IEnumerable<Task> GetAllTasksOrderedByEETThenByName() – returns all of the tasks ordered by EET in descending order, then by length of name in ascending order 
...
If the category already exists - throw ArgumentException()
...
If there is no such category - throw ArgumentException()
...
●	IEnumerable<Category> GetChildren(string categoryId) – returns all child (direct + indirect) categories of the category with the given id.
If there is no such category - throw ArgumentException()
...
o	Explanation: If A is parent of B, and B is parent of C, and C is parent of D -> if we request the hierarchy of category D, we should receive a collection containing [A, B, C, D] in this order.
If there is no such category - throw ArgumentException()
