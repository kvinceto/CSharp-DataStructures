Data Structures Advanced with C# Exam – 21.11.2020
...
This DogVet works with Dog & Owner entities, all dogs and owners are identified by their unique Ids (there will not be two dogs/owners with the same Id). A Owner cannot own two dogs with the same Name.  Implements all the operations from the interface:
...
This class stores two types of entities: Members and Trainers, those entities are identified by their unique ids (there will not be two members and trainers with the same ids). Implements all the operations from the interface:
...
You can submit code to this problem without full coverage from the previous problem, not all test cases will be considered, only the general behaviour will be considered important, edge cases will mostly be ignored such as throwing exceptions etc…
